...
class SSTRouter {
  ...
  private def routerConfig = RouterConfigDsl[SSTRoute].buildConfig { implicit dsl =>
    import dsl._
    ( notebooksRoute
    ...
    <.div(^.`class` := "ui container",
      <.div(^.`class` := "ui secondary pointing menu",
        <.a(
          ...
          (^.className := "active").when(c.pathFor(r.page).value.startsWith(c.pathFor(NotebooksRoute).value)),
          "Notebooks")
      ),
  ...
  private def notebooksRoute(implicit dsl: RouterConfigDsl[SSTRoute]): dsl.Rule = {
    import dsl._
    staticRoute(root, NotebooksRoute) ~> renderR(ctl => notebooksWrapper(proxy => NotebookList(ctl, NotebooksRoute, proxy)))
  }
    ...
    dynamicRouteCT("#notebooks" / long.caseClass[NotebookDetailsRoute]) ~>
      dynRenderR {
        case (route@NotebookDetailsRoute(id), router) =>
          val wrapper = SSTCircuit.connect(m => NotebookDetails.Model(m.notebooks.flatMap(x => Pot.fromOption(x.items.find(_.id == id))), m.notes))
    ...
    dynamicRouteCT("#notebooks" / (long / "notes" / long).caseClass[NoteDetailsRoute]) ~>
      dynRenderR {
        case (route@NoteDetailsRoute(notebookId, noteId), router) =>
          val wrapper = SSTCircuit.connect(m => NoteDetails.Model(
            m.notebooks.flatMap(x => Pot.fromOption(x.items.find(_.id == notebookId))),
