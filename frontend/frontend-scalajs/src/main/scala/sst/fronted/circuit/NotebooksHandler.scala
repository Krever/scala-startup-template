...
/**
  ...
  */
class NotebooksHandler[M](modelRW: ModelRW[M, Pot[Notebooks]]) extends ActionHandler(modelRW) with LazyLogging {
  override def handle: PartialFunction[Any, ActionResult[M]] = {
    ...
    case UpdateNotebook(notebook) =>
      def effect = ApiClient.updateNotebook((notebook.id, NotebookRequest(notebook.name)))
    ...
    case DeleteNotebook(notebook) =>
      updated(value.map(_.remove(notebook)), Effect(ApiClient.deleteNotebook(notebook.id).toFuture.map(_ => RefreshNotebooks)))
  }
}
